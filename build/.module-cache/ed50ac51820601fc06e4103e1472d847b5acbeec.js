...
var Student = React.createClass({displayName: 'Student',
  render: function(){
    return(
      React.DOM.div( {className:"student-container col-lg-4"}, 
        React.DOM.div( {className:"img-container"}

        ),
        React.DOM.div( {className:"content-container row"}, 
          React.DOM.div( {className:"col-sm-7 col-sm-offset-5"}, 
            "Name Goes here"
          ),
...
var Students = React.createClass({displayName: 'Students',
  render: function(){
    ...
    $.each(students, function(index, val) {
      profiles.push(Student(null ))
    });
    return(
      React.DOM.div( {className:"profile-container"}, 
        profiles
      )
    )
  }
