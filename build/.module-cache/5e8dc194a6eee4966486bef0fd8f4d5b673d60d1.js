...
var Student = React.createClass({displayName: 'Student',
  render: function(){
    ...
    $.each( (this.props.data.links || []), function(index, val) {
      if(index == "linkedin"){
        links.push(
          React.DOM.a( {href:val, className:"profile-link"} ,  " ", React.DOM.img( {className:"img-responsive", src:"img/linkedin.png"} ), " " )
        )
      }
      else if(index == "email"){
        var mailto = "mailto:" + val;
        links.push(
          React.DOM.a( {href:mailto, className:"profile-link"} ,  " ", React.DOM.img( {className:"img-responsive", src:"img/linkedin.png"} ), " " )
        );
  ...
  componentDidMount : function(){
    ...
    student.hover(function() {
      student.find('.profile_picture').animate({height: 150, width: 150}, 400);
    }, function() {
      student.find('.profile_picture').animate({height: "100%", width: "100%"}, 400);
    });
  }
});
