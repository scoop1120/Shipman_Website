...
var EventPicture = React.createClass({displayName: 'EventPicture',
  render : function(){
    var url = "event_pictures/" + this.props.picture_url;
    return(
      React.DOM.div( {className:"event-picture-container col-md-6 col-sm-12"}, 
        React.DOM.img( {className:"img-responsive", src:this.props.picture_url} )
      )
    )
  }
}); 
...
var Event = React.createClass({displayName: 'Event',
  render: function(){
    ...
    $.each((this.props.data || []), function(index, val) {
      pictures.push(EventPicture( {picture_url:val.picture_url} ))
    });
    return(
      React.DOM.div( {className:"event-container"}, 
        React.DOM.h2(null,  " ", this.props.data.title, " ", React.DOM.small(null, this.props.data.date)),
        React.DOM.p( {className:"event-description"},  " " ),
